    ...
    public List<Complaint> Affichage() {
        ...
        List<Complaint> ComplaintsList = new ArrayList<>();
         try {
            ...
            while (rs.next()) {
                Complaint C = new Complaint(rs.getInt(1),rs.getString(2), rs.getString(3), rs.getInt(4),fromDate(rs.getDate(5)),rs.getString(6),rs.getString(7),rs.getString(8));
 ...
 @Override
      public  void chargeTableauDonnees(TableView table) {
         ...
         try{
            ...
            String sqlp = "SELECT complaint_id,title,description,employee_id,date_complaint,status,answer FROM complaint";
   ...
   public void recherche(String v, TableView tableProjet) {
    ...
    public List<Complaint> recherche2(String v, ListView liste) {
    String requete = "SELECT * FROM complaint WHERE complaint_id LIKE '%"+v+"%' OR title LIKE '%"+v+"%' or status LIKE '%"+v+"%'  ORDER BY complaint_id DESC ";
    ...
    public void repondre(int id,String reponse) {
          try {
            String req = "UPDATE complaint SET answer=?,status=?,seen=?  WHERE complaint_id=?";
            
            ps = connection.prepareStatement(req);
            
            ps.setString(1, reponse);
            ps.setString(2,"terminé");
            ps.setString(3,"non vue");
            ps.setInt(4,id);
            ps.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(Employee_Services.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public Complaint getComplaint_info_ById(Integer id) {
         String req = "select complaint_id,title,description,employee_id,answer,seen from complaint where complaint_id =?";
     Complaint c = null;
         try {
            ps = connection.prepareStatement(req);
            ps.setInt(1, id);
            rs = ps.executeQuery();
            while (rs.next()) {
                c= new Complaint(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getString(5));
            }
        } catch (Exception x) {
            x.printStackTrace();
        }
    ...
    {try {
        String req = "UPDATE complaint SET seen=?  WHERE complaint_id=?";
        ps = connection.prepareStatement(req);
         ps.setString(1,"vu");
        ps.setInt(2, id);
    ...
    public int NbrNotSeen(int id_emp)
    { int res= 0;
     try {
         String req = "select count(*) from complaint where seen =? and employee_id=?";
         ps = connection.prepareStatement(req);
         ps.setString(1,"non vue");
         ps.setInt(2, id_emp);
         rs= ps.executeQuery();
         while (rs.next()) {
                 res= rs.getInt(1);
